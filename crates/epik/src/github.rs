//! Talking to GitHub.
//!
//! One hand-rolled client, on the [`Client`] precedent: the protocol is
//! spoken directly, the surface covers exactly Epik's verbs, and
...
impl Merge {
    // Driven by merge_pull under `native`; compiled everywhere serde is.
...
pub enum Error {
    ...
    /// rather than a fault: paste a PAT into Settings.
    TokenAbsent,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TokenAbsent => write!(
                f,
                "no GitHub token: this needs one — set the GitHub token in Settings (Cmd+,)"
            ),
...
/// authenticated agent. Built fresh each turn, which is how a PAT pasted
/// mid-session reaches the next turn's verbs with no shared cell.
#[cfg(feature = "native")]
#[derive(Debug)]
pub struct GitHub {
    api: String,
...
/// authorization header to either kind of request builder. The one place
/// the token's bytes leave their [`Secret`], straight into the header.
#[cfg(feature = "native")]
trait Bearer {
    #[must_use]
    fn bearer(self, token: Option<&Secret>) -> Self;
}
...
impl<B> Bearer for ureq::RequestBuilder<B> {
    fn bearer(self, token: Option<&Secret>) -> Self {
        match token {
            Some(token) => self.header("authorization", format!("Bearer {}", token.reveal())),
            None => self,
        }
    }
}
...
// Driven by the client under `native`; compiled — and tested —
// everywhere serde is.
#[cfg_attr(not(feature = "native"), allow(dead_code))]
...
mod tests {
    ...
    #[test]
    fn the_refusal_for_a_missing_token_points_at_settings() {
        let rendered = Error::TokenAbsent.to_string();
        assert!(rendered.contains("Settings (Cmd+,)"), "{rendered}");
    }
